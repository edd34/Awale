...
public class LevelSelectButton : MonoBehaviour {
	...
	//	if (LevelCode == "11") 
		...
		if(!unlocked){
			Color myColor = transform.Find("ButtonGUI").GetComponent<SpriteRenderer>().color;
			myColor = new Color(myColor.r, myColor.g,myColor.b, 0.5f);
			transform.Find("ButtonGUI").GetComponent<SpriteRenderer>().color = myColor;
			transform.Find("ButtonGUI").GetComponent<ButtonEvent>().inactive = true;
		}
		
		if(beaten){
			transform.Find("ButtonGUI").Find("BeatenSprite").GetComponent<SpriteRenderer>().enabled = true;
		}
		if(beatenPar){
			transform.Find("ButtonGUI").Find("BeatenSprite").Find("BeatenParSprite").GetComponent<SpriteRenderer>().enabled = true;
		}
